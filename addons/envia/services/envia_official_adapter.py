...
class EnviaOfficialAdapter(EnviaAdapterBase):
    ...
    def quote(self, request: QuoteRequest) -> QuoteResponse:
        ...
        try:
            body = self.client.post(get_envia_checkout_path(self.shop_id), payload)
        ...
        if carriers and (request.carriers or "").strip().lower() != "all":
            services = [service for service in services if service.carrier in carriers]

        expected_drop_off = EnviaOfficialAdapter._resolve_expected_drop_off(request)
        ...
        quote_id = f"checkout_{self.shop_id}_{request.origin_postal_code}_{request.destination_postal_code}_{len(services)}"
    ...
    @staticmethod
    def _parse_generate_response(body: dict[str, Any], carrier: str, service: str) -> CreateShipmentResponse:
    ...
    @staticmethod
    def _pick_cheapest_service(
        services: list[QuoteService],
        expected_drop_off: int | None = None,
    ) -> QuoteService | None:
        if not services:
            return None
        candidates = EnviaOfficialAdapter._prefer_services_for_route(services, expected_drop_off)
