...
class EnviaQuoteWizard(models.TransientModel):
    ...
    def _attach_branch_carrier_rates(self, side):
        ...
        for carrier, proxy_branch in proxy_by_carrier.items():
            ...
            service = EnviaOfficialAdapter._pick_cheapest_service(
                response.services,
                expected_drop_off,
            )
            if service:
    ...
    def _validate_before_quote(self):
        ...
        errors = self._collect_validation_errors(require_branches=False)
        if errors:
            raise UserError("\n".join(errors))

    def _validate_before_label(self):
        self.ensure_one()
        errors = self._collect_validation_errors(require_branches=True)
        if errors:
            raise UserError("\n".join(errors))
