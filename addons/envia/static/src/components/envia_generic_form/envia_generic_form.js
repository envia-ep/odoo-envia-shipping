...
export class EnviaGenericFormDialog extends Component {
    ...
    async applyInitialValues() {
        ...
        if (initial.phone_code) {
            this.state.phoneCode = String(initial.phone_code);
        }
        if (initial.identity && typeof initial.identity === "object") {
            const next = { ...this.state.identity };
            for (const [key, raw] of Object.entries(initial.identity)) {
                if (raw == null || raw === "") {
                    continue;
                }
                next[key] = String(raw);
            }
            this.state.identity = next;
        }
