...
from odoo.addons.envia.services.envia_config import get_envia_shop_address_base_url
...
class TestEnviaWarehouseOrigin(TransactionCase):
    ...
    def test_get_shop_default_addresses_uses_shop_base_and_token(self, mock_get):
        ...
        base = get_envia_shop_address_base_url()
        client = EnviaClient(base, "shipping-token")
        addresses = client.get_shop_default_addresses("34107", base_url=base)
    ...
    def test_settings_opens_wizard(self):
        company = self.env.company
        company.write(
            {
                "envia_shop_id": "34107",
        ...
        with patch(
            ...
            action = settings.action_open_envia_warehouse_origin_wizard()
        self.assertEqual(action["res_model"], "envia.warehouse.origin.wizard")
